...
O perfil do cliente da loja (`/perfil/dados`, `/perfil/seguranca`, `/perfil/enderecos` e `/perfil/cartoes`) também está integrado à API via `perfil-api.js` → `clientesService` → `clientesApi`. Como não há autenticação, a sessão da loja escolhe um **cliente real cadastrado no PostgreSQL** (identificador mantido em `localStorage`) e todas as alterações dessas quatro telas persistem no banco com as mesmas validações e auditoria do painel administrativo. Pedidos (`/perfil/pedidos`), cupons e checkout seguem no store demonstrativo.
...
- Cadastro público, checkout, vendas, pagamento, estoque e trocas completos ainda não estão integrados a esta API.
...
| --- | --- | --- |
| CRUD integrado atual | `backend/`, `/admin/clientes`, `/perfil/{dados,seguranca,enderecos,cartoes}`, figuras 18, 19, 23, 24 e 25 | Código executável e validado com PostgreSQL e Selenium |
...
- Integrar cadastro público, pedidos/cupons do perfil e checkout à API real quando essa etapa for solicitada.
